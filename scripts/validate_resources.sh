...
echo "[4/4] Scanning Kotlin files for drawable references..."
MISSING_REFS=0
for ref in $(grep -rh "R.drawable\." app/src/main/java/ | sed -E 's/.*R\.drawable\.([a-zA-Z0-9_]+).*/\1/' | sort -u); do
    if [ ! -f "app/src/main/res/drawable/$ref.png" ] && \
       [ ! -f "app/src/main/res/drawable/$ref.xml" ] && \
       [ ! -f "app/src/main/res/drawable-nodpi/$ref.png" ]; then
        echo "WARNING/ERROR: Referenced R.drawable.$ref not found in res/drawable!"
...
if [ $MISSING_REFS -gt 0 ]; then
    ...
    echo "  ✓ All code R.drawable.* references match existing drawable resources."
fi
