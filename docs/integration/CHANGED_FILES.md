...
- `.github/workflows/build.yml` | Bump `runs-on: ubuntu-22.04` to `runs-on: ubuntu-24.04` for the `build` job | DOES SPACE MUSEUM FAIL TO WORK WITHOUT THIS? Yes — a real CI run (commit a748edf, check-run 104372100009, diagnostic instrumentation added to the "Install matc" step) proved the pinned Filament 1.71.5 linux matc binary requires GLIBC_2.38 ("`/lib/x86_64-linux-gnu/libc.so.6: version 'GLIBC_2.38' not found`"), while ubuntu-22.04 only ships glibc 2.35. ubuntu-24.04 ships glibc 2.39 (satisfies matc) and its preinstalled Android SDK image includes build-tools 36.0.0/36.1.0 and platform android-36.1, matching this project's real compileSdk 36.1 (app/build.gradle.kts) — so no other toolchain pin changes (AGP/Kotlin/Gradle/JDK) are needed. This is a real environment fix, not a workaround that avoids running matc.
