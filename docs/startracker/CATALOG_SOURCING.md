...
- **Catalog size vs. performance:** 9k stars → pairs within 40° cutoff ~ N²×0.058 ≈ 4.7M pairs (per earlier extrapolation), quads nearby limited ~ N×19600/4 ≈ 44M quads worst-case, but with magnitude and region filtering, realistically few million. Binary file size: MEASURED by CatalogSerializerTest (2026-09-03 remediation) - 9k stars serialize to 4,067,730,000 bytes = 3.79 GB and 15k stars to 7,182,750,000 bytes = 6.69 GB (quad term dominates: 3.70/6.17 GB). The earlier '~10-30 MB' estimate (Task 4 extrapolation) was wrong - it was an unexecuted guess whose arithmetic also hid an Int overflow; see PHASE3_4_5_6_FINAL_REPORT.md corrections. NOT acceptable as a bundled Android asset at this representation; shrinking requires a fundamentally smaller encoding (dedup/hash-based quad keys, tighter numeric formats) and is future work.
...
**Why not larger:** Including fainter stars (mag 8-9, ~50k stars) would increase catalog size and pair/quad count quadratically, making index larger (on the order of 20+ GB at the CURRENT serialization - see the measured 3.79 GB @ 9k / 6.69 GB @ 15k above) and search slower, with little benefit since phone cannot detect those faint stars.
