...
## ADDENDUM (final pass, 2026-09-04): OD6 — fix APPLIED

Per the final-pass OD6 decision (supersedes the pass-3 "report only" verdict above), the
R3-B4 fix is now applied:

- `OrientationProvider` gains a dedicated, non-conflatable channel:
  `sensorTimestampNanos: StateFlow<Long>` (`_sensorTimestampNanos`), updated on every
  sensor tick right before `_orientation.value = ...`.
- Reader R1 (`CompassARScreen`) now collects that flow
  (`val sensorTimestampNanos by orientationProvider.sensorTimestampNanos.collectAsState()`
  + `LaunchedEffect(sensorTimestampNanos)`) instead of keying on the equality-excluded
  `SkyOrientation.timestampNanos` field, so the feed no longer freezes on a stationary
  device while R2-A1(b)'s conflation behavior is preserved for everything else.

Classification: OD6 calls this KIND-A-equivalent. The changed reader feeds only the
star-tracker clock-domain cross-check (`cameraFrameObserver.onSensorTimestamp`), which
is inert while `StarTrackerConfig.ENABLED = false`. **UNEXECUTED on device** (no
Android runtime in this environment; OrientationProvider/CompassARScreen are not part
of the offline harness compile set) — the diff is syntax-mirrored on existing patterns
in the same files and must be compiled + device-verified per the F runbook.
