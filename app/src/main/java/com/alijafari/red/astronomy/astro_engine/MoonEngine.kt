...
object MoonEngine {
    ...
    data class EarthFromMoonData(
        val phaseNameEn: String,
        val phaseNameFa: String,
        val illuminationPercent: Double,
        val angularDiameterArcmin: Double,
        val distanceKm: Double,
        val subsolarLatitude: Double,
        val subsolarLongitude: Double,
        val subEarthLatitudeOnMoon: Double,
        val subEarthLongitudeOnMoon: Double,
        val earthAltitudeInLunarSky: Double,
        val earthAzimuthInLunarSky: Double,
        val isVisibleFromObserver: Boolean,
        val earthRaDeg: Double,
        val earthDecDeg: Double,
        val phaseAngleDeg: Double
    )

    fun calculateEarthFromMoon(
        jd: Double,
        observerLunarLat: Double = 0.67, // Default Sea of Tranquility / Apollo 11
        observerLunarLon: Double = 23.47
    ): EarthFromMoonData {
        val moonData = calculateMoon(jd)
        val earthIlluminationPercent = (100.0 - moonData.illuminationPercent).coerceIn(0.0, 100.0)
        val phaseAngleDeg = (180.0 - Math.toDegrees(moonData.phaseAngleRad)).coerceIn(0.0, 180.0)

        // Determine Earth phase name (opposite of Moon phase)
        val (phaseEn, phaseFa) = when {
            earthIlluminationPercent >= 97.0 -> "Full Earth" to "زمین کامل"
            earthIlluminationPercent <= 3.0 -> "New Earth" to "زمین نو (تاریک)"
            earthIlluminationPercent in 47.0..53.0 -> {
                if (moonData.ageDays < 14.76) "Third Quarter Earth" to "تربیع آخر زمین"
                else "First Quarter Earth" to "تربیع اول زمین"
            }
            earthIlluminationPercent > 50.0 -> {
                if (moonData.ageDays < 14.76) "Waning Gibbous Earth" to "زمین گوژپشت کاهنده"
                else "Waxing Gibbous Earth" to "زمین گوژپشت افزاینده"
            }
            else -> {
                if (moonData.ageDays < 14.76) "Waning Crescent Earth" to "زمین هلال کاهنده"
                else "Waxing Crescent Earth" to "زمین هلال افزاینده"
            }
        }

        // Earth angular diameter viewed from Moon (~1.9 degrees = ~114 arcmin)
        val angularDiameterArcmin = (2.0 * atan(6371.0 / moonData.distanceKm) * (180.0 / PI) * 60.0)

        // Sun position at this timestamp
        val sunPos = SunEngine.calculatePosition(jd)
        val gmstDeg = TimeEngine.getGMST(jd)
        var subsolarLon = (180.0 - gmstDeg) % 360.0
        if (subsolarLon > 180.0) subsolarLon -= 360.0
        if (subsolarLon < -180.0) subsolarLon += 360.0
        val subsolarLat = sunPos.decDeg

        // Sub-Earth point on Moon (libration)
        val subEarthLat = moonData.librationLatDeg
        val subEarthLon = moonData.librationLonDeg

        // Great circle distance on Moon between observer and sub-Earth point
        val obsLatRad = Math.toRadians(observerLunarLat)
        val obsLonRad = Math.toRadians(observerLunarLon)
        val subLatRad = Math.toRadians(subEarthLat)
        val subLonRad = Math.toRadians(subEarthLon)

        val cosC = sin(obsLatRad) * sin(subLatRad) + cos(obsLatRad) * cos(subLatRad) * cos(obsLonRad - subLonRad)
        val angularDistRad = acos(cosC.coerceIn(-1.0, 1.0))
        val earthAltitude = 90.0 - Math.toDegrees(angularDistRad)

        // Azimuth from lunar observer toward sub-Earth point
        val y = sin(subLonRad - obsLonRad) * cos(subLatRad)
        val x = cos(obsLatRad) * sin(subLatRad) - sin(obsLatRad) * cos(subLatRad) * cos(subLonRad - obsLonRad)
        var earthAzimuth = Math.toDegrees(atan2(y, x))
        if (earthAzimuth < 0) earthAzimuth += 360.0

        // Earth is visible if altitude > -3.0 degrees (considering libration & limb refraction)
        val isVisible = earthAltitude >= -3.0

        // Earth RA / Dec as seen from Moon is opposite to Moon RA / Dec
        var earthRa = (moonData.raDeg + 180.0) % 360.0
        val earthDec = -moonData.decDeg

        return EarthFromMoonData(
            phaseNameEn = phaseEn,
            phaseNameFa = phaseFa,
            illuminationPercent = earthIlluminationPercent,
            angularDiameterArcmin = angularDiameterArcmin,
            distanceKm = moonData.distanceKm,
            subsolarLatitude = subsolarLat,
            subsolarLongitude = subsolarLon,
            subEarthLatitudeOnMoon = subEarthLat,
            subEarthLongitudeOnMoon = subEarthLon,
            earthAltitudeInLunarSky = earthAltitude,
            earthAzimuthInLunarSky = earthAzimuth,
            isVisibleFromObserver = isVisible,
            earthRaDeg = earthRa,
            earthDecDeg = earthDec,
