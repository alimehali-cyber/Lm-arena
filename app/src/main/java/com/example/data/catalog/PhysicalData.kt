...
object PhysicalData {
    ...
    fun getPhysicalProperties(obj: CelestialObject): PhysicalProperties {
        return physicalMap[obj.id] ?: PhysicalProperties(
            diameterKm = 10000.0,
            diameterDisplayFa = "تخمین‌زده بر اساس داده‌های استاندارد catalog",
            diameterDisplayEn = "Estimated based on catalog data",
            massKgDisplayFa = "داده جرم در کاتالوگ موجود است",
            massKgDisplayEn = "Mass data available in catalog",
            gravityMssDisplayFa = "گرانش سطحی محاسبه‌شده",
            gravityMssDisplayEn = "Calculated surface gravity",
    ...
    fun getCoolFactsFa(obj: CelestialObject): List<String> {
        return coolFactsMap[obj.id] ?: listOf(
            "این جرم یکی از برترین اهداف رصدی در کاتالوگ نجومی RED است.",
            "برای رصد دقیق‌تر، استفاده از دوربین دوچشمی یا تلسکوپ آماتوری پیشنهاد می‌شود.",
            "موقعیت زاویه‌ای آن در آسمان بر اساس محاسبات لحظه‌ای اپفمریس مشخص می‌شود.",
            "بهترین زمان برای رصد این جرم هنگام عبور از نصف‌النهار (اوج ارتفاع) است.",
            "کنتراست رصدی آن در آسمان تاریک کویری (بورتل ۱ تا ۳) به حداکثر می‌رسد."
        )
    }
}
